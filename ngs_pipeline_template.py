...
with DAG(
    ...
    def send_prom_metrics(push_gateway_address, **kwargs):
        ...
        labelnames = {parameters} + ["job_uuid"]
        g = Gauge('k8s_cluster_setup', 'GKE cluster setup', labelnames=labelnames, registry=registry)
        g.labels({parameters_with_values}, job_uuid=dag_run_uuid).set(time.time())
    ...
    alignment = SparkKubernetesOperator(
        ...
        application_file="resources/seqtender/{alignment_spark_config}.yaml",
        do_xcom_push=True,
        retries=8,
    ...
    annotation = SparkKubernetesOperator(
        ...
        namespace='default',
        kubernetes_conn_id="kubernetes_default",
        application_file="resources/seqtender/{annotation_spark_config}.yaml",
        do_xcom_push=True,
        retries=8,
        retry_delay=timedelta(seconds=30),
        dag=dag
